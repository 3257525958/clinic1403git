document.querySelectorAll('.input-group input').forEach(input => {
    input.addEventListener('input', function() {
        const icon = this.previousElementSibling; // دریافت آیکون
        if (this.value !== "") {
            icon.style.display = 'none'; // مخفی کردن آیکون
        } else {
            icon.style.display = 'inline'; // نمایش دوباره آیکون زمانی که فیلد خالی است
        }
    });
});
